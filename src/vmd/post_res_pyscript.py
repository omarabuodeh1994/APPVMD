...
def vmd_analysis(file_path,veh_class,bridge_name,num_passes,modes_included):
    ...
    for pass_num in range(1,num_passes+1):  
        ...
        f_bounds = get_freq_bounds(f,sel_psd_off,idx_max_psd_off,modes_included)
        ...
        for mode_num in range(modes_included):
            for i in range(num_sensors):
                sig_final_off[:,i,mode_num] = butter_filter(sel_dec_off[:,i,mode_num], [f_bounds[0][mode_num][i],f_bounds[1][mode_num][i]], 'bandstop', fs, 8)
            for i in range(num_sensors):
                sig_final_on[:,i,mode_num] = butter_filter(sel_dec_on[:,i,mode_num], [f_bounds[0][mode_num][i],f_bounds[1][mode_num][i]], 'bandstop', fs, 8)      
...
if __name__ == '__main__':
    ...
    # (2) File Inputs:
    file_path = '../../parametric_studies/mult_veh/case_studies/'
