...
if __name__ == '__main__':
    ...
    case_studies = ['original','med_span','long_span','med_thick','large_thick','low_num',
    ...
    for veh_class in veh_classes:
        for case_study in case_studies:
            ...
            off_bridge_path = file_path+veh_class+'/'+case_study+'/off_bridge'
            os.makedirs(off_bridge_path,exist_ok=True)
            # on-bridge directory:
            on_bridge_path = file_path+veh_class+'/'+case_study+'/on_bridge'
            os.makedirs(on_bridge_path,exist_ok=True)
            # vmd signal directory:
            vmd_sigs_path = file_path+veh_class+'/'+case_study+'/vmd_sigs'
            ...
            with open (status_path, "a") as file_object:
                file_object.write(f'case study: {case_study}\n')
            # Initialize final dataframes for each sensor:
            df_dom_veh_final = pd.DataFrame()
            df_on_concat,df_off_concat = pd.DataFrame(),pd.DataFrame()
            for pass_num in range(1,num_passes+1):
                # print status of pass number in txt file:
                with open (status_path, "a") as file_object:
                    file_object.write(f'pass num: {pass_num}\n')
            ...
            df_dom_veh_final.reset_index(drop=True,inplace=True)
            ...
            df_stat['case'] = df_stat.shape[0]*[case_study]
            # (26) Plot histograms:
            fig_on,hist_on = get_hist_freq(df_on_concat,['front_axle','rear_axle'],bin_width,'On-bridge')
            fig_on.savefig(file_path+veh_class+'/'+case_study+'/set_diff_'+case_study+'.pdf')
            fig_on.clf();hist_on.cla()
            fig_off,hist_off = get_hist_freq(df_off_concat,['front_axle','rear_axle'],bin_width,'Off-bridge')
            fig_off.savefig(file_path+veh_class+'/'+case_study+'/set_diff_'+case_study+'.pdf')
            fig_off.clf();hist_off.cla()
            # (27) Save results to csv file:
            # save set difference:
            df_diff.to_csv(file_path+veh_class+'/'+case_study+'/set_diff_'+case_study+'.csv',index=False)
            # save statistics:
            df_stat.to_csv(file_path+veh_class+'/'+case_study+'/stat_'+case_study+'.csv',index=False)