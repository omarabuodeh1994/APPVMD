...
def vmd_each_veh_pass(file_path,bridge_name,pass_num,fs,low_freq,freq_res,num_peaks,modes_included,veh_vel):
    mode_names = gen_mode_num(modes_included)
    off_bridge_path = file_path+'sedan/' + bridge_name + '/off_bridge_V_'+str(veh_vel)
    on_bridge_path = file_path+'sedan/' + bridge_name + '/on_bridge_V_'+str(veh_vel)
    ...
    df_dec_off.to_csv(file_path+'sedan/'+bridge_name+'/vmd_sigs_V_'+str(veh_vel)+'/dec_sigs_off_'+str(pass_num)+'.csv',index=False)
    df_dec_on = list_to_df(dec_sig_on,sensors)
    df_dec_on.to_csv(file_path+'sedan/'+bridge_name+'/vmd_sigs_V_'+str(veh_vel)+'/dec_sigs_on_'+str(pass_num)+'.csv',index=False)
    # (7) Select first mode:
    # off-bridge:
...
if __name__ == '__main__':
    ...
    for case_study in case_studies:
        file_path = '../../parametric_study_velocity/case_studies/'+case_study
        for bridge_name in bridge_names:
            # print status of case study in txt file:
            status_path = file_path+'sedan/status_python.txt'
            for veh_vel in V:
                ...
                vmd_sigs_path = file_path + 'sedan/' + bridge_name + '/vmd_sigs_V_'+str(veh_vel)
                ...
                vmd_runs_dict = Parallel(n_jobs=10,prefer='processes')(delayed(vmd_each_veh_pass)(file_path,bridge_name,pass_num,fs,low_freq,freq_res,num_peaks,modes_included,veh_vel) for pass_num in range(1,num_passes+1))
                # concatenate final results into separate dfs:
                df_off_concat,df_on_concat = pd.DataFrame(),pd.DataFrame()
                # vmd_runs_dict = []
                # for pass_num in range(1,num_passes+1):
                #     df_off_temp,df_on_temp = vmd_each_veh_pass(file_path,bridge_name,pass_num,fs,low_freq,freq_res,num_peaks,modes_included,veh_vel)
                ...
                df_stat['veh_class'] = df_stat.shape[0]*['sedan']
                ...
                fig_on.savefig(file_path+'sedan/'+bridge_name+'/hist_on_'+bridge_name+'_V_'+str(veh_vel)+'.pdf')
                fig_on.clf()
                hist_on.cla()
                plt.close(plt.gcf())
                fig_off,hist_off = get_hist_freq(df_off_concat,['front_axle','rear_axle'],bin_width,'Off-bridge')
                fig_off.savefig(file_path+'sedan/'+bridge_name+'/hist_off_'+bridge_name+'_V_'+str(veh_vel)+'.pdf')
                fig_off.clf()
                hist_off.cla()
                plt.close(plt.gcf())
                # (28) Save results to csv file:
                # save set difference:
                df_diff.to_csv(file_path+'sedan/'+bridge_name+'/set_diff_'+bridge_name+'_V_'+str(veh_vel)+'.csv',index=False)
                # save statistics:
                df_stat.to_csv(file_path+'sedan/'+bridge_name+'/stat_'+bridge_name+'_V_'+str(veh_vel)+'.csv',index=False)
                # save dominant frequencies:
                df_stat_dominant.to_csv(file_path+'sedan/'+bridge_name+'/stat_dominant_'+bridge_name+'_V_'+str(veh_vel)+'.csv',index=False)