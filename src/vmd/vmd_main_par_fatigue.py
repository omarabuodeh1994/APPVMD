...
def vmd_each_veh_pass(file_path,veh_class,case_study,pass_num,fs,low_freq,freq_res,num_peaks):
    ...
    for sens in range(num_sensors):
        opt_mode_temp, opt_alpha_temp = get_opt_vmd_params (filt_sig_off[:,sens],[100,2000],[2,7],sig_params_opt)
        opt_mode_off.append(opt_mode_temp)
        opt_alpha_off.append(opt_alpha_temp)
        opt_mode_temp, opt_alpha_temp = get_opt_vmd_params (filt_sig_on[:,sens],[100,2000],[2,7],sig_params_opt)
...
if __name__ == '__main__':
    ...
    V = 10
    # file inputs:
    file_path = '../../parametric_study_4_veh/case_studies/fatigue/'
    ...
    for veh_class in veh_classes:
        ...
        # create directories to store results:
            ...
            vmd_runs_dict = Parallel(n_jobs=10,prefer='processes')(delayed(vmd_each_veh_pass)(file_path,veh_class,bridge_name,pass_num,fs,low_freq,freq_res,num_peaks) for pass_num in range(1,num_passes+1))
            ...
            # Binning dfs for on-bridge data
            # body:
