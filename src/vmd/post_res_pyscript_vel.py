...
# VMD Analysis:
def vmd_analysis(file_path,bridge_name,num_passes,modes_included,veh_vel):
    ...
    for pass_num in range(1,num_passes+1):  
        ...
        csv_file_off = file_path+'sedan'+'/'+bridge_name+'/vmd_sigs_V_'+str(veh_vel)+'/dec_sigs_off_'+str(pass_num)+'.csv'
        csv_file_on = file_path+'sedan'+'/'+bridge_name+'/vmd_sigs_V_'+str(veh_vel)+'/dec_sigs_on_'+str(pass_num)+'.csv'
...
if __name__ == '__main__':
    ...
    for case_study in case_studies:
        for bridge_name, veh_vel in itertools.product(bridge_names, V):
            file_path_final = file_path + case_study + '/'
            df_off_concat,df_on_concat = vmd_analysis(file_path_final,bridge_name,num_passes,modes_included,veh_vel)
            print(f'case study: {case_study}, vehicle class: sedan_{str(veh_vel)}, bridge name: {bridge_name}')
            ...
            df_stat['veh_class'] = f'sedan_{str(veh_vel)}'
            ...
            # save set difference:
            df_diff.to_csv(file_path_final+'sedan'+'/'+bridge_name+'/set_diff_'+bridge_name+'_V_'+str(veh_vel)+'_'+str(modes_included)+'_modes.csv',index=False)
            # save statistics:
            df_stat.to_csv(file_path_final+'sedan'+'/'+bridge_name+'/stat_'+bridge_name+'_V_'+str(veh_vel)+'_'+str(modes_included)+'_modes.csv',index=False)
            # save dominant frequencies:
            df_stat_dominant.to_csv(file_path_final+'sedan'+'/'+bridge_name+'/stat_dominant_'+bridge_name+'_V_'+str(veh_vel)+'_'+str(modes_included)+'_modes.csv',index=False)